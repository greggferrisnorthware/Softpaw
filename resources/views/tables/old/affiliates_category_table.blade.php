...
<span class="awesome">Softpaw <?php echo str_replace('-', ' ', $url[6]); ?> Picks</span>
<table>
    <thead>
        <tr>
            <td>rank</td>
    ...
    <tbody>
        @foreach($search_affiliates as $search_affiliate)   
            @if(str_replace(' ', '-', strtolower($search_affiliate->category->category)) == $url[5] && str_replace(' ', '-', strtolower($search_affiliate->pet->pet)) == $url[4])
                <tr>      
                    <td>{{ $search_affiliate->rank }}</td>
