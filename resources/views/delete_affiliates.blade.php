    ...
    <div class="row justify-content-center">
        <div class="col-md-12">
            ...
            @foreach($featured_affiliates as $featured_affiliate)
                @if($featured_affiliate->id == $url[4])
                    <form action="{{ route('delete-affiliate', $featured_affiliate->id) }}" method="POST">
                        @csrf
                        @method('delete')
                        <input class="form-control red" type="submit" name="submit" value="delete product" />
                    </form>
                @endif
            @endforeach
        </div>
