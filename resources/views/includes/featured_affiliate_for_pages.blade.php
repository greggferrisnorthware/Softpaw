...
<div class="container">
    <div class="row">
        <div class="col-md-12">
            <span class="awesome">Top <?php echo $url[4]; ?> Picks</span>
        </div>
    </div>
    ...
    <div class="row featured justify-content-center">
        ...
        @foreach($featured_affiliates as $featured_affiliate)
            @if($featured_affiliate->pet->pet == $url[4] && $count < 5)
                <div class="col-md-4 col-lg-4 col-xl-3 col-xxl-2">
                    <a class="product" rel="sponsored" href="{{ $featured_affiliate->link }}" target="_blank">
                        ...
                        <div class="image-contain">
                            ...
                            {{-- <span class="{{ $featured_affiliate->affiliate }}"></span> --}}
                            <span class=" {{ $featured_affiliate->brand->brand }}"></span>
                        </div>
                        <div class="product-body">     
                            <span><small class="animal">{{ ucwords($featured_affiliate->pet->pet) }}</small> <small class="cat">{{ ucwords($featured_affiliate->category->category) }}</small> <small class="brand">{{ ucwords($affiliate->brand->brand) }}</small></span>      
